    ...
    return (
      <ScrollView style={{flex: 1}}>
        <List containerStyle={{marginBottom: 40, marginTop: 0, flex: 1}}>
          {
            rawData.map((item) => (
            <TouchableWithoutFeedback key={item.genre_id} onPress={() => Actions.SubCategoryShows({genre: item.genre_name})}>
              <ListItem
                onPress={() => Actions.SubCategoryShows({title: item.genre_name, genre: item.genre_name})}
